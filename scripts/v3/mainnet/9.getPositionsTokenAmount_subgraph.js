...
async function main() {
  ...
  ////////////
  const resultBalanceOf = await NonfungiblePositionManagerContract.balanceOf("0x8c595DA827F4182bC0E3917BccA8e654DF8223E1");
  const nftNums = resultBalanceOf.toNumber();
  console.log(nftNums);
  for (let i = 0 ; i< nftNums; i++){
    const tokenIDRPC = await NonfungiblePositionManagerContract.tokenOfOwnerByIndex("0x8c595DA827F4182bC0E3917BccA8e654DF8223E1", i);
    console.log(i.toString(),tokenIDRPC.toString());
    const result = await axios.post(SUBGRAPH_URL, { query: getPOSITIONS_QUERY(tokenIDRPC.toString()) });
...
function getPOSITIONS_QUERY(tokenId){
  return `
  query MyQuery {
