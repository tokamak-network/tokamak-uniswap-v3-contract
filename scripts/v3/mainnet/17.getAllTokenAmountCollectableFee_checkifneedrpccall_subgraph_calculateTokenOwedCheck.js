...
//const SUBGRAPH_URL = 'https://api.thegraph.com/subgraphs/name/ianlapham/uniswap-v3-gorli';
const SUBGRAPH_URL = 'https://thegraph.titan-goerli.tokamak.network/subgraphs/name/tokamak/titan-uniswap-subgraph';
const owner = "0x942d6ac7a6702bb1852676f3f22aee38bd442e4c";
const Q128 = JSBI.BigInt("340282366920938463463374607431768211456");

async function main() {
  const accounts = await hre.ethers.getSigners();
  let deployer = accounts[0];
  const chainName = hre.network.name;
  if (chainName === 'hardhat' || chainName === 'localhost') {
    deployer = await hre.ethers.getImpersonatedSigner(owner);
  }
  ...
  for (let i = 0; i < postionSnapshotLength - 1; i++){
    ...
    if (id1 === id2){
      ...
      if (positionSnapshot1.liquidity !== positionSnapshot2.liquidity && positionSnapshot1.collectedFeesToken0 === positionSnapshot2.collectedFeesToken0 && positionSnapshot1.collectedFeesToken1 === positionSnapshot2.collectedFeesToken1){
        let tokenId = parseInt(id1);
        console.log("tokenId", tokenId);
        //console.log(positionSnapshot1.liquidity, positionSnapshot2.liquidity, positionSnapshot1.withdrawnToken0, positionSnapshot2.withdrawnToken0, positionSnapshot1.withdrawnToken1, positionSnapshot2.withdrawnToken1, positionSnapshot1.collectedFeesToken0, positionSnapshot2.collectedFeesToken0, positionSnapshot1.collectedFeesToken1, positionSnapshot2.collectedFeesToken1)
        // let positionInfo = await NonfungiblePositionManagerContract.positions(tokenId);
        const positionLiquidity = JSBI.BigInt(positionSnapshot2.liquidity);
        const positionFeeGrowthInside0LastX128 = JSBI.BigInt(positionSnapshot2.feeGrowthInside0LastX128);
        const positionFeeGrowthInside1LastX128 = JSBI.BigInt(positionSnapshot2.feeGrowthInside1LastX128);
        const feeGrowthInside0xLastX128 = JSBI.BigInt(positionSnapshot1.feeGrowthInside0LastX128);
        const feeGrowthInside1xLastX128 = JSBI.BigInt(positionSnapshot1.feeGrowthInside1LastX128);
        const tokenOwed0Already2 = JSBI.divide(JSBI.multiply(JSBI.subtract(feeGrowthInside0xLastX128, positionFeeGrowthInside0LastX128), positionLiquidity), Q128);
        const tokenOwed1Already2 = JSBI.divide(JSBI.multiply(JSBI.subtract(feeGrowthInside1xLastX128, positionFeeGrowthInside1LastX128), positionLiquidity), Q128);

        let positionInfo = await NonfungiblePositionManagerContract.positions(tokenId);
        const tokenOwed0Already = positionInfo.tokensOwed0;
        const tokenOwed1Already = positionInfo.tokensOwed1;
        rpccallCount++;
        console.log("rpcCall",tokenId, tokenOwed0Already.toString(), tokenOwed1Already.toString());
        console.log("SubgraphCall",tokenId, tokenOwed0Already2.toString(), tokenOwed1Already2.toString());
  ...
  for (const [key, value] of Object.entries(collectableFees)) {
    ...
    console.log("tokenId", key, "amount0(subgraph)", value[0], "amount0(rpc)", results2.amount0.toString(), "amount1(subgraph)", value[1], "amount1(rpc)", results2.amount1.toString(), 'amount0 correct?',value[0] === results2.amount0.toString(), 'amount1 correct?',value[1]===results2.amount1.toString());
  }
