...
function getLiquidityForAmount1(sqrtRatioAX96, sqrtRatioBX96, amount1) {
  ...
  console.log(FixedPoint96Q96.toString(), sqrtRatioBX96.toString(), sqrtRatioAX96.toString(), amount1.toString());
...
async function addLiquidity(
  ...
  // console.log(    ethers.BigNumber.from(
  //   getSqrtRatioAtTick(nearestUsableTick(tickCurrent - 5108, 60)).toString()
  // ).toString(),     ethers.BigNumber.from(getSqrtRatioAtTick(tickCurrent).toString()).toString(),
  // amount1Desired.toString());
  const liquidityForAmount1 = getLiquidityForAmount1(
    ethers.BigNumber.from(
      getSqrtRatioAtTick(nearestUsableTick(tickCurrent - 5108, 60)).toString()
    ),
    ethers.BigNumber.from(getSqrtRatioAtTick(tickCurrent).toString()),
    amount1Desired
  );
  // console.log(sqrtPriceX96, liquidityForAmount1, amount0Desired.toString());
  // console.log(
  //   JSBI.leftShift(JSBI.BigInt(liquidityForAmount1), JSBI.BigInt(96)).toString()
  // );
  // const product = JSBI.multiply(
  ...
  // );
  const add = false;
  // console.log(    JSBI.BigInt(sqrtPriceX96).toString(),
  // JSBI.BigInt(liquidityForAmount1).toString(),
  // JSBI.BigInt(amount0Desired.toString()).toString());
