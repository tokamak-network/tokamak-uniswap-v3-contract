...
async function main() {
  ...
  const SwapRouterAddress = SwapRouterContract.address;
    ...
    ///=========== TOSContract
    let balanceBeforeUSDC = await USDCContract.balanceOf(deployer.address);
    console.log('balanceBeforeTOS', balanceBeforeUSDC.toString());
...
//     }
    ...
    // console.log(positionInfo);
    // if (positionInfo.liquidity > 0) {
    //   await exitForETH(NonfungiblePositionManagerContract, positionInfo.liquidity, 4, 0, 0, deployer.address, USDCAddress)
    // }
    ...
    ///=========== TOSContract
    let balanceAfterUSDC = await USDCContract.balanceOf(deployer.address);
    console.log('balanceAfterUSDC', balanceAfterUSDC.toString());
    ///=========== ETHContract
    ...
    let poolAddressWETHUSDC = await getPoolContractAddress(UniswapV3FactoryContract, USDCAddress, WETHAddress, 500);
...
//     console.log(await UniswapV3PoolContract.slot0());
    ...
    if(chainName ==='hardhat'){
      ...
      console.log("liquidity: ", await contract.getLiquidityForAmount1(
          getSqrtRatioAtTick(nearestUsableTick(-200101 -5108, 10)).toString(),
          getSqrtRatioAtTick(-200101 - 1).toString(),
  ...
  if (poolAddressWETHUSDC !== '0x0000000000000000000000000000000000000000') {
    ...
    if(sqrtPriceX96.gt(ethers.constants.Zero)){
      ...
      if (liquidity.gt(ethers.constants.Zero)) {
        ...
        let tickUpper = nearestUsableTick(-199158, 60);
        // let tickLower = -600;
        // let tickUpper = 600;
        if(WETHAddress < USDCAddress){
          token0 = WETHAddress;
