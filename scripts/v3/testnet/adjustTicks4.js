...
async function main() {
  ...
  const UniswapV3FactoryContract = await getContract('UniswapV3Factory');
    ...
    ///=============== USDCContract  
    const USDContract = await getContract('USDC');
    const USDCAddress = USDContract.address;
  ...
  const SwapRouterAddress = SwapRouterContract.address;
    ...
    // ///=========== USDTContract
    // let balanceBeforeUSDT = await USDTContract.balanceOf(deployer.address);
    // console.log('balanceBeforeUSDT', balanceBeforeUSDT.toString());
...
//     }
    ...
    // ///=========== USDTContract
    // let balanceAfterUSDT = await USDTContract.balanceOf(deployer.address);
    // console.log('balanceAfterUSDT', balanceAfterUSDT.toString());
    // ///=========== ETHContract
    ...
    let poolAddressWETHUSDC = await getPoolContractAddress(UniswapV3FactoryContract, WETHAddress,USDCAddress,  100);
    ...
    let poolAddressWETHUSDC500 = await getPoolContractAddress(UniswapV3FactoryContract, USDCAddress, WETHAddress, 500);
    let UniswapV3PoolContract500 = UniswapV3Pool_.attach(poolAddressWETHUSDC500);
    console.log(await UniswapV3PoolContract500.slot0());
  ...
  //     console.log("poolAddressWETHUSDC liquidity:", liquidity, slot0.tick);
  //     if (liquidity.eq(ethers.constants.Zero)) {
  //       let deadline = Date.now() + 100000;
  //       console.log(await UniswapV3PoolContract.token0());
  //       console.log(await UniswapV3PoolContract.token1());
        
  //       let tickLower = nearestUsableTick(getTickAtSqrtRatio(JSBI.BigInt(shouldSqrtPricex96)) - 60, 10);
  //       let tickUpper = nearestUsableTick(slot0.tick + 60, 10);
  ...
  //       } else {
  //         token0 = USDCAddress;
  //         amount0Desired = ethers.utils.parseUnits('1', 6);
  //         token1 = WETHAddress;
...
//     console.log(await UniswapV3PoolContract.slot0());
    ...
    if(chainName ==='hardhat'){
      ...
      console.log("liquidity: ", await contract.getLiquidityForAmount1(
          ...
          getSqrtRatioAtTick(-200894 - 1).toString(),
      ...
      const add = true;
      console.log( liquidity, amount, add);
      console.log()
      const nextSqrtPrice = (getNextSqrtPriceFromAmount0RoundingUp(JSBI.BigInt(3441451464131621656007193), JSBI.BigInt(liquidity), JSBI.BigInt(amount), false));
      console.log(nextSqrtPrice.toString());
      console.log("next Tick : ", getTickAtSqrtRatio(nextSqrtPrice).toString());

      const nextSqrtPrice1 = (getNextSqrtPriceFromAmount1RoundingDown(JSBI.BigInt('3441451464131621656007193'), JSBI.BigInt(liquidity), JSBI.BigInt('5000000000'), false));
      console.log(nextSqrtPrice1.toString());

      console.log("next Tick : ", getTickAtSqrtRatio(nextSqrtPrice1).toString());

    }
//-200025 nextTick
positionInfo = await NonfungiblePositionManagerContract.positions(19);
   await exitForETH(NonfungiblePositionManagerContract, positionInfo.liquidity, 19, 0, 0, deployer.address, USDCAddress)
//
  //=======================poolAddressWETHUSDC
  if (poolAddressWETHUSDC !== '0x0000000000000000000000000000000000000000') {
    const UniswapV3Pool_ = new ethers.ContractFactory(
    ...
    if(sqrtPriceX96.gt(ethers.constants.Zero)){
      ...
      if (liquidity.eq(ethers.constants.Zero)) {
        ...
        let tickLower = nearestUsableTick(slot0.tick -5108, 1);
        let tickUpper = nearestUsableTick(-200025, 1);
        // let tickLower = -600;
        // let tickUpper = 600;
        if(WETHAddress < USDCAddress){
          token0 = WETHAddress;
          ...
          token1 = USDCAddress;
          amount1Desired =  ethers.utils.parseUnits('5000', 6);
        } else {
          token0 = USDCAddress;
