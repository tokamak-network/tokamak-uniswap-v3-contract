...
const setup = async() => {
    ...
    TONContract = new ethers.Contract(TON, erc20ABI, l2Signer)
    TOSContract = new ethers.Contract(TOS, erc20ABI, l2Signer)
...
async function main() {
    ...
    if(allowance.lt(allowanceAmount)){
      const tx = await TONContract.connect(l2Signer).approve(
        SwapRouter.address, ethers.utils.parseEther("10000")
        );
      console.log("tx", tx);
    }
    ...
    // const estimation = await QuoterV2.estimateGas.quoteExactInput(
    //     path,
    //     amountIn
    // );
